...
struct diagonal_matrix
{
    ...
    template <typename T>
    explicit diagonal_matrix(const T& d) : d_(d.size())
    {
        std::copy(d.begin(), d.end(), d_.begin());
    }

    diagonal_matrix(const size_type& n) : diagonal_matrix(n, n, 0.0) {}
    ...
    diagonal_matrix(const size_type& n, const size_type& m, const value_type& v)
        : d_(n)
    {
        if (m != n)
...
inline size_t
num_rows(const diagonal_matrix& m)
{
    return m.size1();
}

inline size_t
num_cols(const diagonal_matrix& m)
{
    return m.size2();
}
}
